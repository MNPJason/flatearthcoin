...
### Send 1000 FLAT to New Addresses

Send exactly 1000 FLAT to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1000 FLAT.

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 FLAT on the remote server and defeats the purpose of a hot/cold setup.*
...
## Update flatearth.conf on server

If you generated a new masternode private key, you will need to update the remote `flatearth.conf` files.

Shut down the daemon and then edit the file.

```vim .flatearthcore/flatearth.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `flatearth.conf` file.
...
```flatearth-cli getinfo```

and comparing with the official explorer at https://explorer.flatearthcrypto.com
...
```
flatearth-cli masternode status
{
    "outpoint" : "<collateral_output>-<collateral_output_index>",
    "service" : "<ipaddress>:<port>",
    "pubkey" : "<1000 FLAT address>",
