...
pkgdatadir = $(datadir)/flatearthcore
pkgincludedir = $(includedir)/flatearthcore
pkglibdir = $(libdir)/flatearthcore
pkglibexecdir = $(libexecdir)/flatearthcore
...
CONFIG_HEADER = $(top_builddir)/src/config/flatearth-config.h
CONFIG_CLEAN_FILES = libflatearthconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libflatearthconsensus.pc.in \
	...
	$(top_srcdir)/src/config/flatearth-config.h.in \
...
ACLOCAL = ${SHELL} /media/sf_jerry/coindev/flatearthcoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /media/sf_jerry/coindev/flatearthcoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /media/sf_jerry/coindev/flatearthcoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /media/sf_jerry/coindev/flatearthcoin/build-aux/missing automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /media/sf_jerry/coindev/flatearthcoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = flatearthcore
PACKAGE_BUGREPORT = https://github.com/flatearthcrypto/flatearthcoin/issues
PACKAGE_NAME = FlatEarth Core
PACKAGE_STRING = FlatEarth Core 0.12.2
PACKAGE_TARNAME = flatearthcore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /media/sf_jerry/coindev/flatearthcoin
abs_srcdir = /media/sf_jerry/coindev/flatearthcoin
abs_top_builddir = /media/sf_jerry/coindev/flatearthcoin
abs_top_srcdir = /media/sf_jerry/coindev/flatearthcoin
ac_ct_AR = ar
...
install_sh = ${SHELL} /media/sf_jerry/coindev/flatearthcoin/build-aux/install-sh
...
pkgconfig_DATA = libflatearthconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/flatearthd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/flatearth-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/flatearth-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = FlatEarth-Qt.app
OSX_DMG = FlatEarth-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_flatearth_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_flatearth_coverage.info test_flatearth.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/FlatEarth-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/flatearth-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/flatearth-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/flatearth-config.h
$(top_srcdir)/src/config/flatearth-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/flatearth-config.h src/config/stamp-h1
libflatearthconsensus.pc: $(top_builddir)/config.status $(srcdir)/libflatearthconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/FlatEarth-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/FlatEarth-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "FlatEarth-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/FlatEarth-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_flatearth.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_flatearth -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_flatearth_filtered.info: test_flatearth.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_flatearth_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_flatearth_filtered.info
...
#test_flatearth_coverage.info: baseline_filtered_combined.info test_flatearth_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_flatearth_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_flatearth_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_flatearth_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_flatearth.coverage/.dirstamp:  test_flatearth_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_flatearth.coverage/.dirstamp total.coverage/.dirstamp

# FlatEarth: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_flatearth.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
