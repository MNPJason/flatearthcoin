...
#if defined(NDEBUG)
# error "FlatEarth Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in nFLAT) are considered zero fee (for relaying, mining and transaction creation) */
...
const string strMessageMagic = "FlatEarthCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        return 100000 * COIN; // FlatEarth project fund 1%
    }
    
       
    nSubsidyBase = 6;	//reward 6 FLAT / 2 min
	
    CAmount nSubsidy = nSubsidyBase * COIN;

    //supply total 10,000,000 FLAT
    if(nPrevHeight >= 1650000)
        nSubsidy = 0;    

    return nSubsidy;
}

//coinstew - controls MN payout
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{        
    CAmount ret = blockValue * 0.95; // 95% for Masternodes
    return ret;
}
...
void ThreadScriptCheck() {
    RenameThread("flatearth-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // FLATEARTH : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(FLAT): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(FLAT): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // FLATEARTH : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(FLAT): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(FLAT): spork is off, skipping transaction locking checks\n");
    }
