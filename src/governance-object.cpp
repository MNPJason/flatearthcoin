...
bool CGovernanceObject::IsValidLocally(std::string& strError, bool& fMissingMasternode, bool& fMissingConfirmations, bool fCheckCollateral)
{
    ...
    // IF FLATEARTH NO COUNT (NO-YES VALID VOTES) IS MORE THAN 10% OF THE NETWORK MASTERNODES, OBJ IS INVALID
        ...
        if((nObjectType == GOVERNANCE_OBJECT_TRIGGER) || (nObjectType == GOVERNANCE_OBJECT_WATCHDOG)) {
            ...
            if(!mnodeman.GetMasternodeInfo(vinMasternode.prevout, infoMn)) {
                ...
                if (err == CMasternode::COLLATERAL_OK) {
                    ...
                    strError = "Masternode UTXO should have 1000 FLAT, missing masternode=" + strOutpoint + "\n";
                }
...
int CGovernanceObject::GetFlatEarthYesCount(vote_signal_enum_t eVoteSignalIn) const
{
    return GetYesCount(eVoteSignalIn) - GetNoCount(eVoteSignalIn);
}

int CGovernanceObject::GetFlatEarthNoCount(vote_signal_enum_t eVoteSignalIn) const
{
...
{
    return CountMatchingVotes(eVoteSignalIn, VOTE_OUTCOME_FLATTAIN);
}
...
void CGovernanceObject::UpdateSentinelVariables()
{
    ...
    if(GetFlatEarthYesCount(VOTE_SIGNAL_FUNDING) >= nAbsVoteReq) fCachedFunding = true;
    if((GetFlatEarthYesCount(VOTE_SIGNAL_DELETE) >= nAbsDeleteReq) && !fCachedDelete) {
        fCachedDelete = true;
        if(nDeletionTime == 0) {
            nDeletionTime = GetAdjustedTime();
        }
    }
    if(GetFlatEarthYesCount(VOTE_SIGNAL_ENDORSED) >= nAbsVoteReq) fCachedEndorsed = true;

    if(GetFlatEarthNoCount(VOTE_SIGNAL_VALID) >= nAbsVoteReq) fCachedValid = false;
}
