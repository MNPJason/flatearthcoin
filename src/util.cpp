...
const char * const BITCOIN_CONF_FILENAME = "flatearth.conf";
const char * const BITCOIN_PID_FILENAME = "flatearthd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "flatearth" is a composite category enabling all FlatEarth-related debug output
            if(ptrCategory->count(string("flatearth"))) {
...
#else
    const char* pszModule = "flatearth";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FlatEarthCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FlatEarthCore
    // Mac: ~/Library/Application Support/FlatEarthCore
    // Unix: ~/.flatearthcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FlatEarthCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FlatEarthCore";
#else
    // Unix
    return pathRet / ".flatearthcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty flatearth.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override flatearth.conf
